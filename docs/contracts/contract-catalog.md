...
| `S2cCallbackTransport` | tenant-scoped | `S2cCallbackEnvelope.tenantId` in-band required field, validated non-blank (Rule R-C.c). Stage 2 tenant migration (2026-06-14, agent-bus-stage1-review-and-stage2-plan.md) promoted tenant from the prior out-of-band registry-resolution design (ADR-0074 §Consequences) to an in-band field on the envelope; the registry binding is retained as a COMPATIBILITY ONLY path and MUST NOT substitute for the in-band scope. Resolves deferred AUD-2026-05-27 AUD-EVT-6 / family `F-cross-authority-tenant-scope-claim-without-field`. The record now carries 9 components `{callbackId, tenantId, serverRunId, capabilityRef, requestPayload, traceId, idempotencyKey, deadline, requestAttributes}` | unchanged — relocated to agent-bus per ADR-0088 |
...
| `S2cCallbackEnvelope` / `S2cCallbackResponse` | `agent-bus` (`...bus.spi.s2c`) | Seven mandatory request fields per ADR-0074 (six original + `tenantId` added in the Stage 2 tenant migration, Rule R-C.c); relocated per ADR-0088 |
