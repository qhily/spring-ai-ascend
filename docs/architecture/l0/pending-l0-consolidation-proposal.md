...
| L0-CONFLICT-008 | Some old reading paths still point readers primarily at legacy `ARCHITECTURE.md` files instead of the new L0 package. | `README.md`, older docs. | Update reading paths to reference the new L0 package while treating `docs/archive/ARCHITECTURE.md` as historical source material. |
...
`docs/archive/ARCHITECTURE.md` has not been fully absorbed by the new L0
document package.
...
- Six L0 logical modules as future L1 domains; reactor/build artifacts remain
  downstream development or deployment concerns.
...
|---|---|---|---|
| ARCH-UNABSORBED-001 | The full 65 numbered L0 constraints with per-item wording, ADR references, rule/enforcer references, and wave qualifiers. | The new `constraints.md` groups constraints but does not preserve all precise normative wording. | Use `docs/archive/ARCHITECTURE.md` as historical source until each constraint is migrated, superseded, or linked to generated rule/enforcer facts. |
...
| ARCH-UNABSORBED-010 | Freeze marker and edit-path compliance language for old root `ARCHITECTURE.md`. | Historical context for why the old corpus was protected during consolidation. | Superseded by the 2026-06-02 user decision to archive the old root file while preserving it as historical source material. |

## Recommendation

The legacy `architecture/docs/L0/ARCHITECTURE.md` has been moved to
`docs/archive/ARCHITECTURE.md` by user decision on 2026-06-02.
...
4. Task canonical vocabulary is propagated to downstream L1/L2 and contract docs.
5. Root and L1 reading paths are updated to the new package.

After that, the archived corpus can remain as decision history or be split into
more precise governance, L1/L2, contract, and verification references.
