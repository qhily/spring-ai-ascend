...
| 转发边界 | 类 MQ 转发底座仍是设计态；消费 Stage 3 route handle；broker / MQ 产品选择 deferred；大载荷走 data reference path，不进 event / control channel（见 [`ICD-Agent-Bus-Forwarding`](../../../../docs/architecture/l0/05-contracts/human-readable/ICD-agent-bus-forwarding.md)）。 |
...
- 类 MQ 转发底座 runtime（broker / queue / DLQ / replay 存储）。
- service discovery API。

### 5.1 运行态候选评审（Stage 5）

Stage 5 对类 MQ 转发底座的运行态承载候选（in-memory dispatcher / runtime-local queue / database outbox-inbox / external broker / hybrid）做了 broker-agnostic 的候选评审，**不实现运行态、不绑定 broker 产品**。评审见 [`agent-bus-forwarding-runtime-candidates`](../../../../docs/architecture/l0/10-governance/review-packets/agent-bus-forwarding-runtime-candidates.md)。上表中的 broker 产品、queue / DLQ / replay 存储、broker topic 绑定等仍是 runtime 物理决策，需 H2/H3 裁决后由 Stage 6 最小运行态切片设计承接，不在当前 L1 草案的已实现事实中。

### 5.2 运行态候选裁决（Stage 6）

Stage 6 建立运行态候选裁决记录 [`agent-bus-forwarding-runtime-decision`](../../../../docs/architecture/l0/10-governance/review-packets/agent-bus-forwarding-runtime-decision.md)，**状态 draft，等待 H2/H3 裁决**：是否采用某候选（默认推荐 C3、备选 C1、暂不推荐 C5，均非裁决）、是否允许写生产代码。**裁决前不得写生产 runtime 代码、不引入 broker 依赖、不新增 outbox / DLQ / replay store**（Stage 6 计划 §6）。§5 表中的所有物理实现仍是「实现等待裁决」，不在当前 L1 草案的已实现事实中。
