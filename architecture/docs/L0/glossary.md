...
| Task | Unified server-side authoritative execution lifecycle state for V1. It aligns with A2A protocol task semantics and can be created or bound by a client-to-server request or by an `agent-service` A2A request to another `agent-service`. | `agent-service` | Session, Memory, client invocation, engine-internal execution state | accepted |
...
| Heterogeneous framework compatibility | Engines and adapters from different agent frameworks can participate through governed SPI, adapter, hook, and contract surfaces. | `agent-execution-engine`, `agent-middleware`, `agent-bus` per accepted placement | Closed single-framework runtime or lifecycle-owner rewrite | candidate_promote |
...
| Task tree | Parent-child execution relationship used to trace delegation, join, failure, and cost attribution. | `agent-service` + observability | Single trace span or engine-internal state | accepted |
